...
public final class Screen: @unchecked Sendable
{
    ...
    //
    public func scale(scaling: Bool = true) -> CGFloat { scaling ? _scale : 1.0 }
    //
    ...
    // These functions scale the given value as appropriate; there are versions which take integer or floating
    // point; as well as ones which take a scaling boolean indicating whether or not to really scale the value.
    //
    public func scaled(_ value: Int) -> Int { Int(round(CGFloat(value) * _scale)) }
    public func scaled(_ value: CGFloat) -> CGFloat { value * _scale }
    public func scaled(_ value: Int, scaling: Bool) -> Int { scaling ? Int(round(CGFloat(value) * _scale)) : value }
    public func scaled(_ value: CGFloat, scaling: Bool) -> CGFloat { scaling ? value * _scale : value }

    public func unscaled(_ value: Int) -> Int { Int(round(CGFloat(value) / _scale)) }
    public func unscaled(_ value: CGFloat) -> CGFloat { value / _scale }
    public func unscaled(_ value: Int, scaling: Bool) -> Int { scaling ? Int(round(CGFloat(value) / _scale)) : value }
    public func unscaled(_ value: CGFloat, scaling: Bool) -> CGFloat { scaling ? value / _scale : value }
}
